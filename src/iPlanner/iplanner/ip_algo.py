...
class IPlannerAlgo:
    ...
    def plan(self, image, goal_robot_frame):
        ...
        img = self.depth_transform(img).expand(1, 3, -1, -1).float()
        # tensor_stats(img, "planner_input_img")
...
def tensor_stats(x, name="tensor"):
    ...
    print(
        ...
        f"dtype={x_cpu.dtype}, "
        f"device={x.device}, "
        f"min={x_cpu.min().item():.6f}, "
        f"max={x_cpu.max().item():.6f}, "
        f"mean={x_cpu.mean().item():.6f}, "
        f"std={x_cpu.std().item():.6f}, "
        f"nan={torch.isnan(x_cpu).any().item()}, "
        f"inf={torch.isinf(x_cpu).any().item()}"
    )
